# Changelog
- 2026-07-02 — **Conciliar cobradas · revisión histórica del stock (mes a mes)**. Nuevo `CobradasSinRespaldoModal` (botón "Cobradas s/ respaldo · N" en la barra de Conciliación bancaria): toma el STOCK de facturas cobradas (Pagado) SIN respaldo bancario de un año (default 2026) y las cruza con la cartola cargada (abono del mismo cliente/RUT, monto exacto, fecha cercana al pago). Las separa en **calce exacto** (con botón "Conciliar los N" en lote, sin re-usar un mismo abono) / **por revisar** (varios exactos o solo cercanos → eliges) / **sin pago** (no está en la cartola cargada), agrupadas por mes. Concilia reusando `reconciliar` (fuente única; una Pagado sin respaldo tiene saldoFactura=monto, así que se liga sin cambiar estado ni mandar correo). Cada fila muestra la comparación factura ↔ depósito (monto exacto/dif · ±días · glosa). Verificado en demo (render + agrupación por mes, sin crash). **A verificar en prod** con cartola real (toca cifras, con compuerta). Build verde.
- 2026-07-02 — **Cargar cartola: paso de confirmación antes de guardar**. El cargador ya no escribe directo: parsea y muestra una compuerta "Revisa antes de guardar" con la cuenta detectada (Cta. Gastos/Honorarios · N°), el período (fecha min–max), Movimientos · Nuevos · Ya cargados (dedup por hash), Abonos +/Cargos − y si cuadra con el banco, más el botón que dice exacto "Cargar N movimientos en Cta. Gastos" (o Descartar). `onFiles` separado en preparar (parsea, NO escribe → `cargaPreview`) y `confirmarCarga` (escribe solo los nuevos, ignoreDuplicates; el reporte posterior queda igual). Evita cargar mal sin darse cuenta. Build verde, demo monta.
- 2026-07-02 — **Emisión SII: PDF + XML se guardan solos en Drive y quedan adjuntos a la factura**. Al emitir un DTE con éxito, `subirFacturaADrive` reusa el generador existente (`facturaDtePdfBase64`, con logo + timbre) y la carpeta de facturación de Drive (`folders.facturas`) —mismo camino que `procesarRespaldoSII`— para subir el PDF y el XML y adjuntarlos a la factura (`billing_attachments`, "Emisión SII"), visibles desde Facturación y la ficha del cliente. Idempotente por nombre, no bloquea la emisión (si Drive no está conectado, lo avisa). Antes el PDF se generaba al vuelo sin guardarse y solo el XML se subía vía respaldo manual. **A verificar en prod** (emisión real + Drive conectado; el demo no emite). Build verde.
- 2026-07-02 — **Cobertura SII: chequeo de facturas del año sin cliente**. Nueva fila en el hub de Conciliación → modal `CoberturaSIIModal` (read-only + asignar). Por año: foto con protagonista "Sin cliente · N · $monto" (fantasmas por asignar) o "✓ Todas con cliente", más "Emitidas en el sistema" y "Cobertura %". Lista las emitidas (folio real, no programada/anulada) con `client_id` nulo, agrupadas por mes, cada una con RS·RUT·concepto y asignación en un toque (sugerencia por RUT vía `resolverClienteSII` + buscador `AsignarClienteInline`, reusa `handleAssignClient` que aprende el RUT↔cliente). Puente "Cotejar con el SII" para las que faltan del portal. Verificado en demo (14 emitidas, 100%). Build verde.
- 2026-07-02 — **Conciliación en lote: filas enriquecidas**. Las filas de "calzan exacto" pasan de "cliente → Factura N°X · $monto" a dos líneas con la info disponible: cliente · fecha del depósito · RUT, y debajo → Factura N° · emisión · concepto. Las de "por revisar" suman fecha y RUT. Mismo criterio de mostrar todo lo comparable. Build verde.
- 2026-07-02 — **Conciliación bancaria: conciliar una factura de gastos/reembolso**. En el calce sugerido, botón "Es reembolso ▾" abre dos opciones (el usuario elige caso a caso): "Solo saldar (no ingreso)" o "Saldar + fondo por rendir". Ambas marcan la factura `billing_type='reembolso'` (única palanca que la excluye del ingreso/por-cobrar) y la concilian pagada con respaldo; la segunda además crea un fondo por rendir del cliente por el monto. Antes esta opción no existía y una factura de gastos se contaba como ingreso al conciliarla. **A verificar en prod** (toca cifras; el demo no trae cartola). Build verde.
- 2026-07-02 — **Conciliación bancaria: calce en paralelo enriquecido (depósito ↔ factura)**. Las tarjetas de calce (sugerido individual y "pago en grupo") pasan de una línea pobre a una comparación en dos columnas: a la izquierda el depósito (monto, fecha, cuenta, RUT, nombre, N° op., glosa) y a la derecha la factura (N°, monto/saldo, emisión, RS·RUT, estado, concepto). En grupo, cada depósito y cada factura por separado con su subtotal + "cuadra exacto" solo si las sumas coinciden al peso; RUT y RS en cada línea para confirmar que todo es del mismo RUT. Chips de por qué calza (mismo RUT · monto exacto). Presentación pura — misma lógica (reconciliar/reconciliarGrupo). Detecta factura de reembolso (badge + botón "Conciliar (reembolso)"). Build verde, demo monta.
- 2026-07-02 — **Motor del cotejo (match.ts): los recurrentes ya no se auto-calzan al mes equivocado**. La raíz de los enredos: el auto-match filtraba las programadas **al mes del cotejo** (`due startsWith periodo`), así que a un recurrente (mismo monto todos los meses) le "encontraba 1" —la del mes cotejado— y le pegaba el folio al mes equivocado (mes vencido: la de julio es servicio de junio). Ahora busca candidatos en **todos** los periodos; si hay más de uno **o alguno es recurrente** (mensual/permanente), va a **"Elige la factura correcta"** (manual, con el mes a la vista). Solo se automatiza lo único y no recurrente. Espejo del `matchProgEmitidas` del frontend. Desplegado; a confirmar con un cotejo real.
- 2026-07-02 — **Cotejar SII: "Ponerle el folio" avisa claro cuando el folio ya existe (no error crudo)**. Si el folio del SII ya está en otra factura tuya —incluso una retirada/eliminada, que la constraint `billing_invoice_no_unique` igual cuenta—, "Poner N°382" ya no muestra el error de base de datos: avisa *"El folio N°382 ya existe en otra factura tuya (puede estar retirada); revísalo en Limpiar duplicados o repón la factura correcta"*. Guardián client-side + `catch` que detecta el 23505. Diagnóstico: los folios 382 (MISION) / 381 (Better) del cotejo ya vivían en las facturas correctas ("Junio", 01/07) que se habían eliminado por error → se reponen por SQL; constraint sugerida a índice parcial `where deleted_at is null`. Build verde.
- 2026-07-02 — **Cotejar SII: "Ponerle el folio" con comparación de dos columnas (consistencia)**. Se le dio a "Ponerle el folio del SII" el MISMO formato comparable desplegable que "Confirma el cliente" / "Elige la factura": la fila se abre y muestra **En el SII** (N°382 · monto · fecha · RUT · RS) ↔ **Tu factura** (folio actual/sin folio · monto · estado · concepto), con el botón adentro. Así en TODO el cotejo comparás igual. Build verde.
- 2026-07-02 — **Cotejar SII: "Corregir folio" ahora se explica solo**. El nombre y "Asignar Factura N°382" eran crípticos. Renombrado a **"Ponerle el folio del SII"**; cada fila muestra "Tu factura sin folio · $monto · concepto" ↔ "En el SII salió con N°382 · fecha · RUT", y el botón dice **"Poner N°382"** (o "Cambiar a N°382" si tenía folio manual). Deja claro que solo le pega a tu factura el folio real del SII, no crea nada. Build verde.
- 2026-07-02 — **Cotejar SII "Elige la factura": ofrece TODAS las cuotas de la venta, en orden (no salta la 2/3)**. El motor mezclaba "programadas del período" + "vencidas sin folio" y podía omitir la cuota del medio (ofrecía 1/3 y 3/3, faltando la 2/3 correcta). Ahora, en el front, los candidatos se expanden a **todas las cuotas SIN emitir de la venta** de cada candidata (por `sale_id`), ordenadas por N° de cuota → aparecen 1/3, 2/3, 3/3 en orden y eliges la correcta. Build verde; SII no monta en demo, probar en prod.
- 2026-07-02 — **Limpiar duplicados: solo duplicados (fuera "Sin proyecto asignado")**. La sección "Sin proyecto asignado" (que sugería una venta para facturas huérfanas — no es un duplicado, y ofrecía cruces "nada que ver" como Honorarios genéricos del 31-jul) se sacó de la vista. "Limpiar duplicados" ahora muestra SOLO duplicados reales (copias sin folio, folio repetido, programadas ya emitidas). Empty state: "Sin duplicados". Build verde, verificado en demo.
- 2026-07-02 — **Limpiar duplicados: 3 correcciones tras revisión real**. (1) **Una factura con folio real NUNCA es duplicada** — el detector `dupGroups` ya no agrupa como duplicadas dos facturas con folios reales DISTINTOS (ej. una cuota dividida en 3 facturas iguales, N°249/250/251); solo elimina fantasmas (sin folio) o el mismo folio repetido. (2) **Secuencias por cuota** (Cuota N/M) también quedan fuera del auto-match (como los mensuales) → Luis Silva "Cuota 2/10" ya no se ofrece contra la programada equivocada. (3) **Más info para comparar**: cada fila SE CONSERVA / SE ELIMINA muestra ahora concepto · razón social · fecha (antes solo folio+monto). Build verde; verificado en demo (Transportes Volcán, folios distintos, ya no se marca duplicado).
