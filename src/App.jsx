...
const Inp = (p) => <input {...p} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:`1px solid ${C.border}`,background:'#F5F7F9',color:C.text,fontSize:14,boxSizing:'border-box',outline:'none',...p.style}}/>
const Sel = ({value,onChange,options,placeholder}) => (
  <select value={value} onChange={onChange} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:`1px solid ${C.border}`,background:'#F5F7F9',color:C.text,fontSize:14,boxSizing:'border-box'}}>
    {placeholder&&<option value=''>{placeholder}</option>}
    {options.map(o=><option key={o} value={o}>{o}</option>)}
  </select>
)
const Txt = (p) => <textarea {...p} rows={2} style={{width:'100%',padding:'10px 12px',borderRadius:8,border:`1px solid ${C.border}`,background:'#F5F7F9',color:C.text,fontSize:14,resize:'none',boxSizing:'border-box',outline:'none'}}/>
const Lbl = ({children}) => <div style={{fontSize:11,fontWeight:600,color:C.muted,textTransform:'uppercase',letterSpacing:.7,marginBottom:3}}>{children}</div>
const Fld = ({label,children,mb}) => <div className='fld' style={{marginBottom:mb??10}}><Lbl>{label}</Lbl>{children}</div>
              ...
              return (
                <div key={t.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 0',borderBottom:`1px solid ${C.border}`}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
                    ...
                    <span style={{fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:.4,padding:'2px 7px',borderRadius:4,background:vigente?'#E1F5EE':'#E4E8EB',color:vigente?'#0F6E56':C.muted}}>{vigente?'Vigente':'Histórico'}</span>
                  </div>
                </div>
              )
            })}
              ...
              <div key={b.id} onClick={()=>onEdit(b)} style={{position:'relative',background:C.card,borderRadius:10,padding:'11px 13px',marginBottom:6,border:`1px solid ${C.border}`,cursor:'pointer'}}>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8,marginTop:7}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',minWidth:0}}>
                    ...
                    {anticipada&&<span style={{fontSize:10,padding:'1px 7px',borderRadius:10,background:'#E1F5EE',color:'#0F6E56',fontWeight:600}}>Anticipada</span>}
                  </div>
                  <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
